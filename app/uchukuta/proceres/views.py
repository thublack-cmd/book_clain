...
from app.models import db, Clain_uchu_proceres
from app.mail import send_mail_open
from . import proceres

now = (date.today()).strftime('%d-%m-%Y')
now2 = (datetime.utcnow()).strftime('%Y-%m-%dT%H:%M:%S')
sala = 'proceres'

@proceres.route('/cliente', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        # add serial to table
        last_clain = Clain_uchu_proceres.query.order_by(Clain_uchu_proceres.created_at.desc()).first()
        if not last_clain:
            serial = 'SUP-1'
        else:
            serial = 'SUP-' + str(last_clain.id)

        new_clain = Clain_uchu_proceres(
                serial = serial,
        ...
        return redirect(url_for('proceres.client_view'))
...
@proceres.route('/', methods=['POST', 'GET'])
@login_required
def audit_view():
...
@proceres.route('/<int:id>/descargo')
@login_required
