...
from app.models import db, Clain_uchu_huacho
...
@huacho.route('/cliente', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        # add serial to table
        last_clain = Clain_uchu_huacho.query.order_by(Clain_uchu_huacho.created_at.desc()).first()
        if not last_clain:
            serial = 'SUH-1'
        else:
            serial = 'SUH-' + str(last_clain.id)

        new_clain = Clain_uchu_huacho(
                serial = serial,
        ...
        return redirect(url_for('huacho.client_view'))
