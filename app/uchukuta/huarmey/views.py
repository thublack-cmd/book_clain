...
from app.models import db, Clain_uchu_huarmey
...
@huarmey.route('/cliente', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        # add serial to table
        last_clain = Clain_uchu_huarmey.query.order_by(Clain_uchu_huarmey.created_at.desc()).first()
        if not last_clain:
            serial = 'SUM-1'
        else:
            serial = 'SUM-' + str(last_clain.id)

        new_clain = Clain_uchu_huarmey(
                serial = serial,
...
@huarmey.route('/', methods=['POST', 'GET'])
@login_required
def audit_view():
