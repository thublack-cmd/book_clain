...
from app.models import db, Clain_uchu_zarate
...
@zarate.route('/cliente', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        # add serial to table
        last_clain = Clain_uchu_zarate.query.order_by(Clain_uchu_zarate.created_at.desc()).first()
        if not last_clain:
            serial = 'SUZ-1'
        else:
            serial = 'SUZ-' + str(last_clain.id)

        new_clain = Clain_uchu_zarate(
                serial = serial,
        ...
        return redirect(url_for('zarate.client_view'))
