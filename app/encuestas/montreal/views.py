...
@emontreal.route('/', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        for i in range(1, 500):
            value = sh.worksheet("Montreal").get('A' + str(i))
            if bool(value) == False:
                sh.worksheet("Montreal").update('A' + str(i) + ':P' + str(i),
        ...
        return redirect(url_for('emontreal.client_view'))
