...
@ekavari.route('/', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        for i in range(1, 500):
            value = sh.worksheet("Kavari").get('A' + str(i))
            if bool(value) == False:
                sh.worksheet("Kavari").update('A' + str(i) + ':P' + str(i),
        ...
        return redirect(url_for('ekavari.client_view'))
