...
@etribeca.route('/', methods=['POST', 'GET'])
def client_view():
    if request.method == 'POST':
        ...
        for i in range(1, 500):
            value = sh.worksheet("Tribeca").get('A' + str(i))
            if bool(value) == False:
                sh.worksheet("Tribeca").update('A' + str(i) + ':P' + str(i),
        ...
        return redirect(url_for('etribeca.client_view'))
